...
aes_cbc = AES(CBC_key, 'CBC')
for ct in CBC_cts:
    print(aes_cbc.decrypt(ct))
...
aes_ctr = AES(CTR_key, 'CTR')
for ct in CTR_cts:
    print(aes_ctr.decrypt(ct))
...
aes_ctr = AES(key, 'CTR')
assert(aes_ctr.decrypt(aes_ctr.encrypt(message, iv)) == message)
aes_cbc = AES(key, 'CBC')
assert(aes_cbc.decrypt(aes_cbc.encrypt(message, iv)) == message)
